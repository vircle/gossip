...
CzGOSSTracker::CzGOSSTracker(std::string strWalletFile)
{
...
CzGOSSTracker::~CzGOSSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzGOSSTracker::Init()
{
...
bool CzGOSSTracker::Archive(CMintMeta& meta)
{
...
bool CzGOSSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzGOSSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzGOSSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzGOSSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzGOSSTracker::GetSerialHashes()
{
...
CAmount CzGOSSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzGOSSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzGOSSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzGOSSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzGOSSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzGOSSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzGOSSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzGOSSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzGOSSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzGOSSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzGOSSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGOSSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGOSSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzGOSSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzGOSSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzGOSSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzGOSSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzgoss from DB\n", __func__, listDeterministicDB.size());
    }
