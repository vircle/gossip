...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a GOSSIP address (e.g. %1)").arg("GdjRD6xUKticHaPduiiXwS44NRpE8v7zskgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
void setupIPWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    // We don't want translators to use own addresses in translations
    // and this is the only place, where this address is supplied.
    widget->setPlaceholderText(QObject::tr("Enter a VPS IP (e.g. %1)").arg("127.0.0.2:23677"));
#endif
}
...
{
    // return if URI is not valid or is no GOSSIP: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::GOSS, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert gossip:// to gossip:
    //
    //    Cannot handle this later, because gossip:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::GOSS, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open gossip.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GOSSIP.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for GOSSIP.lnk
...
{
    return GetAutostartDir() / "gossip.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a gossip.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=GOSSIP\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the gossip app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add gossip app to startup item list
