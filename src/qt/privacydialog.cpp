                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zGOSSpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
...
void PrivacyDialog::on_pushButtonzGOSSControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    zGOSSControlDialog* zGOSSControl = new zGOSSControlDialog(this);
    zGOSSControl->setModel(walletModel);
    zGOSSControl->exec();
}

void PrivacyDialog::setzGOSSControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzGOSS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid GOSSIP Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GOSS </b>will be added to the standard transaction fees!<br />";
    ...
    if (!zGOSSControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = zGOSSControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zGOSS require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zGOSS"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZGOSS_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zGOSS require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zGOSS"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zGOSS was spent successfully update the addressbook with the label
    ...
    // Clear zgoss selector in case it was used
    zGOSSControlDialog::setSelectedMints.clear();
    ui->labelzGOSSSelected_int->setText(QString("0"));
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " GOSS, ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zgosstracker->GetMints(false);
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzGOSS->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        // Mint zGOSS
        ui->pushButtonMintzGOSS->setEnabled(true);
        ui->pushButtonMintzGOSS->setToolTip(tr("PrivacyDialog", "Enter an amount of GOSS to convert to zGOSS", 0));

        // Spend zGOSS
        ui->pushButtonSpendzGOSS->setEnabled(true);
        ui->pushButtonSpendzGOSS->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}