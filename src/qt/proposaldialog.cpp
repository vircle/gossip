...
// Copyright (c) 2019 The GOSSIP Developers
...
ProposalDialog::ProposalDialog(Mode mode, QWidget* parent) : QDialog(parent), ui(new Ui::ProposalDialog), mapper(0), mode(mode), counter(0)
{
    ...
    ui->urlEdit->setPlaceholderText(tr("Valid GOSSIP Forum Proposal URL"));
    ui->urlEdit->setToolTip(tr("Valid GOSSIP Forum Proposal URL"));
    ...
    ui->addressEdit->setPlaceholderText(tr("The GOSSIP public address that will receive the funds"));
    ui->addressEdit->setToolTip(tr("The GOSSIP public address that will receive the funds"));
...
bool ProposalDialog::validateProposal()
{
    ...
    std::string address = ui->addressEdit->text().toStdString();
    if (!IsValidDestinationString(address)) strError = "Invalid GOSSIP address";
