...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGOSSIPAmount"))
        settings.setValue("nAnonymizeGOSSIPAmount", 1000);

    nAnonymizeGOSSIPAmount = settings.value("nAnonymizeGOSSIPAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGOSSIPAmount"))
        SoftSetArg("-anonymizegossipamount", settings.value("nAnonymizeGOSSIPAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in gossip.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGOSSIPAmount:
            return QVariant(nAnonymizeGOSSIPAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGOSSIPAmount:
            nAnonymizeGOSSIPAmount = value.toInt();
            settings.setValue("nAnonymizeGOSSIPAmount", nAnonymizeGOSSIPAmount);
            emit anonymizeGOSSIPAmountChanged(nAnonymizeGOSSIPAmount);
            break;
        case CoinControlFeatures:
