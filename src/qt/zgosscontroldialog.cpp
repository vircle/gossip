...
#include "ui_zgosscontroldialog.h"
...
std::set<std::string> zGOSSControlDialog::setSelectedMints;
std::set<CMintMeta> zGOSSControlDialog::setMints;

zGOSSControlDialog::zGOSSControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::zGOSSControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
zGOSSControlDialog::~zGOSSControlDialog()
{
    delete ui;
}

void zGOSSControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zGOSSControlDialog::updateList()
{
...
void zGOSSControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zGOSSControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzGOSS_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setzGOSSControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zGOSSControlDialog::GetSelectedMints()
{
...
void zGOSSControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
