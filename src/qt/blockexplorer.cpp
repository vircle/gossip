...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::GOSS, nValue);
...
{
    std::string genesisblockhash = "00000de6a05a95fb27a241808eb19636411f91455cb56f9dbb76e04cad966550";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (gossip.conf).");
            QMessageBox::warning(this, "GOSSIP Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
