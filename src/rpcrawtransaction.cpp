...
UniValue getrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "     {\n"
            "       \"value\" : x.xxx,            (numeric) The value in gossip\n"
            "       \"n\" : n,                    (numeric) index\n"
            ...
            "           \"gossipaddress\"        (string) gossip address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\" : \"address\",  (string) the gossip address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!IsValidDestinationString(input.get_str()))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid GOSSIP address: ") + input.get_str());
