...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zgoss that have been in mempool long
                    //and higher priority to zgosss that are large in value
                    ...
                    // zGOSS spends can have very large priority, use non-overflowing safe functions
        ...
        if (fZerocoinActive) {
            ...
            if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                //For the period before v2 activation, zGOSS will be disabled and previous block's checkpoint is all that will be needed
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GOSSIPMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("GOSSIPMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("GOSSIP-Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gossip-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running GOSSIPMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
