...
CzGOSSWallet::CzGOSSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgoss seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGOSSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGOSSSeed_deprecated()) {
                    LogPrintf("%s: Updated zGOSS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgoss seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgoss wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGOSSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGOSSCount(nCountLastUsed);
    else if (!walletdb.ReadZGOSSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGOSSWallet::Lock()
{
    seedMaster = 0;
}

void CzGOSSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGOSSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGOSS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGOSSWallet::LoadMintPoolFromDB()
{
...
void CzGOSSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGOSSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGOSSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgosstracker->HasPubcoinHash(pMint.first)) {
...
bool CzGOSSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGOSS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgosstracker which also adds to database
    pwalletMain->zgosstracker->Add(dMint, true);
...
void CzGOSSWallet::SeedToZGOSS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGOSSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGOSSWallet::GenerateDeterministicZGOSS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGOSSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGOSSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
