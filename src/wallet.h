...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT9999IFMN = 3,
    ONLY_NONDENOMINATED_NOT9999IFMN = 4,            // ONLY_NONDENOMINATED and not 9999 GOSS at the same time
    ONLY_9999 = 5,                                 // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                              // UTXO's that are valid for staking
};

// Possible states for zGOSS send
enum ZerocoinSpendStatus {
    ZGOSS_SPEND_OKAY = 0,                            // No error
    ZGOSS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZGOSS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZGOSS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZGOSS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZGOSS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZGOSS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZGOSS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZGOSS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZGOSS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZGOSS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZGOSS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZGOSS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZGOSS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZGOSS_SPENT_USED_ZGOSS = 14,                      // Coin has already been spend
    ZGOSS_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZGOSS_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void zGOSSBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZGOSSOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzgossAuto) const;
    ...
    */
    ...
    CzGOSSWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzGOSSTracker> zgosstracker;
    ...
    void setZWallet(CzGOSSWallet* zwallet)
    {
        zwalletMain = zwallet;
        zgosstracker = std::unique_ptr<CzGOSSTracker>(new CzGOSSTracker(strWalletFile));
    }

    CzGOSSWallet* getZWallet() { return zwalletMain; }
    ...
    void setzGOSSAutoBackups(bool fEnabled)
    {
