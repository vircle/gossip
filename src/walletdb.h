...
public:
    ...
    bool WriteZGOSSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGOSSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGOSSSeed_deprecated(uint256& seed);
    bool EraseZGOSSSeed();
    bool EraseZGOSSSeed_deprecated();

    bool WriteZGOSSCount(const uint32_t& nCount);
    bool ReadZGOSSCount(uint32_t& nCount);
