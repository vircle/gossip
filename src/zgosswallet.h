...
#ifndef GOSSIP_ZGOSSWALLET_H
#define GOSSIP_ZGOSSWALLET_H
...
public:
    CzGOSSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZGOSS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZGOSS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //GOSSIP_ZGOSSWALLET_H
