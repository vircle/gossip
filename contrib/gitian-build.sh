...
VERSION=
commit=false
url=https://bitbucket.org/GOSSIPCOIN/gossip
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the gossip, gitian-builder, gitian.sigs, and gossip-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/g0ssipcoin/gossip
...
then
    ...
    git clone https://bitbucket.org/GOSSIPCOIN/gossip-detached-sigs.git
...
then
	# Make output folder
	mkdir -p "./gossip-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit gossip=${COMMIT} --url gossip=${url} ../gossip/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../gossip/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/gossip-*.tar.gz build/out/src/gossip-*.tar.gz ../gossip-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit gossip=${COMMIT} --url gossip=${url} ../gossip/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/gossip-*.zip build/out/gossip-*.exe ../gossip-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit gossip=${COMMIT} --url gossip=${url} ../gossip/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/gossip-*.tar.gz build/out/gossip-*.dmg ../gossip-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit gossip=${COMMIT} --url gossip=${url} ../gossip/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../gossip/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/gossip-*.tar.gz build/out/src/gossip-*.tar.gz ../gossip-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../gossip/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/gossip-*win64-setup.exe ../gossip-binaries/${VERSION}
	    mv build/out/gossip-*win32-setup.exe ../gossip-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../gossip/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/gossip-osx-signed.dmg ../gossip-binaries/${VERSION}/gossip-${VERSION}-osx.dmg
	fi
	popd || exit
