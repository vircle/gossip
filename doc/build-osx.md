...
====================================
This guide will show you how to build gossipd (headless client) for OSX.
...
1. Clone the github tree to get the source code and go into the directory.
        ...
        git clone https://bitbucket.org/GOSSIPCOIN/gossip
        cd gossip
...
5.  (Optional) You can also install gossipd to your path:
...
4. Enter "gossip-qt" as project name, enter src/qt as location
...
------------------------
You can ignore this section if you are building `gossipd` for your own use.

gossipd/gossip-cli binaries are not included in the gossip-Qt.app bundle.

If you are building `gossipd` or `gossip-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the GOSSIP-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./gossipd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./gossipd` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=gossiprpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/GOSSIP/gossip.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/GOSSIP/gossip.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/GOSSIP/debug.log

Other commands:
-------

    ./gossipd -daemon # to start the gossip daemon.
    ./gossip-cli --help  # for a list of command-line options.
    ./gossip-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure GOSSIP with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the GOSSIP wallet:
...
You have the choice to build the GUI GOSSIP wallet as a Mac OSX app, described in “How to build the GOSSIP-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the GOSSIP-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find GOSSIP-Qt.dmg inside your GOSSIP folder. Open and install the wallet like any typical Mac app.
...
~/Downloads/GOSSIP/src/qt/gossip-qt